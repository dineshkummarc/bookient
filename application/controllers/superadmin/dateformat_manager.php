...
class dateformat_manager extends Pardco {
	public function __construct()
	{
		...
		$this->load->library('Changestatus');
                $this->load->model('superadmin/dateformat_manager_model');
	...
	if(empty($dateformat_manager_search))
		$dateformat_manager_search = $this->input->post('dateformat_manager_search');
	...
	if(array_key_exists('doSearch',$parameter)){
		if( $dateformat_manager_search =='Search by Date Format')
		{
			$dateformat_manager_search ='';
		}
		if(!empty($dateformat_manager_search)){
		$likeArr['date_format'] = urldecode(trim($dateformat_manager_search));   //<---##

		$searchLink .= "/dateformat_manager_search/$dateformat_manager_search";
		$like = array('date_format'=>$dateformat_manager_search);                  //<---##
		}
	...
	$order_array = $this->ranking->PrepareOriginalArray('date_format','date_format_id','date_format_order');     //<---##

	if(array_key_exists('type',$parameter)){
		$alter_array = $this->ranking->AlterOriginalArray($order_array,$parameter['order'],$parameter['type']);
		$this->ranking->saveRanking($alter_array,'date_format','date_format_id','date_format_order');        //<---##
	}
	...
	if($action == 'delete')
	{
		$this->ranking->deleteRanking($order_array,'date_format','date_format_id','date_format_order',$record_id);    //<---##
		$Is_Process = $this->input->post('Is_Process');
		$msgreport = $this->dateformat_manager_model->deleteCatalog($record_id);
	...
	$SortingSequenceArr=array(1 => "date_format");          //<---##
	$ReturnSortingArr=$this->sorting->Sorting("date_format_order",$SortingSequenceArr,'GO',$OrderByID,$ordertype);  //<---##
	...
	$config_pagination['total_rows'] = $this->dateformat_manager_model->get_TotalRecords($where,$like);
	...
	//main query
	$CatalogArr = $this->dateformat_manager_model->get_AllCatalogArr($parameter['page'],$config_pagination['per_page'],$ReturnSortingArr['OrderBy'],$ordertype,$searchArr,$likeArr);
	...
	for($i=0;$i<count($CatalogArr);$i++){
		if($searched == '' && !array_key_exists('OrderByID',$parameter))
			$CatalogArr[$i]->img_rank = $this->ranking->displayRankingImage($order_array,$CatalogArr[$i]->date_format_order,base_url().'superadmin/dateformat_manager/index/page/'.$parameter['page']);         //<---##
		else
	...
	$data['dateformat_manager_search'] = $dateformat_manager_search;
	...
	$this->load->view('superadmin/header/header');
        ...
        $this->load->view('superadmin/dateformat_manager/dateformat_manager',$data);
            if($Is_Process!='Y') {
