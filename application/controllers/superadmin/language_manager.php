...
class language_manager extends Pardco {
	public function __construct()
	{
		parent::__construct();
		$this->load->helper('url');
		$this->load->library('Changestatus');
                $this->load->model('superadmin/language_manager_model');
	...
	if(empty($language_manager_search))
		$language_manager_search = $this->input->post('language_manager_search');
	...
	if(array_key_exists('doSearch',$parameter)){
		if( $language_manager_search =='Search by Language Name')
		{
			$language_manager_search ='';
		}
		if(!empty($language_manager_search)){
		$likeArr['languages_name'] 	= urldecode(trim($language_manager_search));   //<---##

		$searchLink .= "/language_manager_search/$language_manager_search";
		$like = array('languages_name' => $language_manager_search);                  //<---##
		}
	...
	$order_array = $this->ranking->PrepareOriginalArray('languages','languages_id','languages_order');     //<---##

	if(array_key_exists('type',$parameter)){
		$alter_array = $this->ranking->AlterOriginalArray($order_array,$parameter['order'],$parameter['type']);
		$this->ranking->saveRanking($alter_array,'languages','languages_id','languages_order');        //<---##
	}
	...
	if($action == 'delete')
	{
		$this->ranking->deleteRanking($order_array,'languages','languages_id','languages_order',$record_id);    //<---##
		$Is_Process = $this->input->post('Is_Process');
		$msgreport = $this->language_manager_model->deleteCatalog($record_id);
	...
	if($action == 'activate')
	{
		//$this->changestatus->ChangeStatus($record_id,$statusid);
                $this->language_manager_model->ChangeStatusLANGUAGE($record_id);
	...
	$SortingSequenceArr=array(1 => "languages_name");          //<---##
	$ReturnSortingArr=$this->sorting->Sorting("languages_order",$SortingSequenceArr,'GO',$OrderByID,$ordertype);  //<---##
	...
	$config_pagination['total_rows'] = $this->language_manager_model->get_TotalRecords($where,$like);
	...
	//main query
	$CatalogArr = $this->language_manager_model->get_AllCatalogArr($parameter['page'],$config_pagination['per_page'],$ReturnSortingArr['OrderBy'],$ordertype,$searchArr,$likeArr);
	...
	for($i=0;$i<count($CatalogArr);$i++){
		if($searched == '' && !array_key_exists('OrderByID',$parameter))
			$CatalogArr[$i]->img_rank = $this->ranking->displayRankingImage($order_array,$CatalogArr[$i]->languages_order,base_url().'superadmin/language_manager/index/page/'.$parameter['page']);         //<---##
		else
	...
	$data['language_manager_search'] = $language_manager_search;
	...
	$this->load->view('superadmin/header/header');
        ...
        $this->load->view('superadmin/language_manager/language_manager',$data);
            if($Is_Process!='Y') {
	...
	{
		$this->language_manager_model->EditLANGUAGE();
	}

        public function SaveLANGUAGEAjax()
	{
		//echo '<pre>';print_r($_POST);exit;
		//echo '<pre>';print_r($_FILES);exit;
		if($_FILES['userfile']['name'] != "")
		{
		     $returndata =$this->language_manager_model->do_upload();
		}

		$msgreport = $this->language_manager_model->SaveLANGUAGE($_POST,$_FILES['userfile']['name']);
                $url = 'http://'.$_SERVER['HTTP_HOST']."/superadmin/language_manager/index/IsPreserved/Y/msg/".$msgreport;
		redirect($url);
