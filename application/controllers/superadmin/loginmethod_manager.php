...
class loginmethod_manager extends Pardco {
	 public function __construct()
	{
		parent::__construct();
		$this->load->helper('url');
		$this->load->library('Changestatus');
                $this->load->model('superadmin/loginmethod_manager_model');
	...
	if(array_key_exists('doSearch',$parameter)){
		if( $login_name_search =='Search by Login Name')
		{
			$login_name_search ='';
		}
		if(!empty($login_name_search)){
		$likeArr['login_name'] = urldecode(trim($login_name_search));   //<---##

		$searchLink .= "/login_name_search/$login_name_search";
		$like = array('login_name'=>$login_name_search);                  //<---##
		}
	...
	$order_array = $this->ranking->PrepareOriginalArray('app_logins','login_typ_id','login_order');     //<---##

	if(array_key_exists('type',$parameter)){
		$alter_array = $this->ranking->AlterOriginalArray($order_array,$parameter['order'],$parameter['type']);
		$this->ranking->saveRanking($alter_array,'app_logins','login_typ_id','login_order');        //<---##
	}
	...
	if($action == 'delete')
	{
		$this->ranking->deleteRanking($order_array,'app_logins','login_typ_id','login_order',$record_id);    //<---##
		$Is_Process = $this->input->post('Is_Process');
		$msgreport = $this->loginmethod_manager_model->deleteCatalog($record_id);
	...
	if($action == 'activate')
	{
		//$this->changestatus->ChangeStatus($record_id,$statusid);
                $this->loginmethod_manager_model->ChangeStatusLOGINMETHOD($record_id);
	}
	...
	$SortingSequenceArr=array(1 => "login_name");          //<---##
	$ReturnSortingArr=$this->sorting->Sorting("login_order",$SortingSequenceArr,'GO',$OrderByID,$ordertype);  //<---##
	...
	$config_pagination['total_rows'] = $this->loginmethod_manager_model->get_TotalRecords($where,$like);
	...
	//main query
	$CatalogArr = $this->loginmethod_manager_model->get_AllCatalogArr($parameter['page'],$config_pagination['per_page'],$ReturnSortingArr['OrderBy'],$ordertype,$searchArr,$likeArr);
	...
	for($i=0;$i<count($CatalogArr);$i++){
		if($searched == '' && !array_key_exists('OrderByID',$parameter))
			$CatalogArr[$i]->img_rank = $this->ranking->displayRankingImage($order_array,$CatalogArr[$i]->login_order,base_url().'superadmin/loginmethod_manager/index/page/'.$parameter['page']);         //<---##
		else
	...
	$data['LoginmethodArr'] = $CatalogArr;
	...
	$data['login_name_search'] = $login_name_search;
	...
	$this->load->view('superadmin/header/header');
        ...
        $this->load->view('superadmin/loginmethod_manager/loginmethod_manager',$data);
            if($Is_Process!='Y') {
	...
	function activate(){
        ...
        public function EditLOGINMETHODAjax()
	{
	    $this->loginmethod_manager_model->EditLOGINMETHOD();
	}

        public function SaveLOGINMETHODAjax()
	{
		$msgreport = $this->loginmethod_manager_model->SaveLOGINMETHOD();
                echo $msgreport;
