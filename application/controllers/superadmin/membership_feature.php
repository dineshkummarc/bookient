...
class membership_feature extends Pardco {
	...
	public function __construct(){
		...
		$this->load->model('superadmin/membership_feature_model');
	...
	function index(){
            ...
            if(empty($membership_feature_search))
                    $membership_feature_search = $this->input->post('membership_feature_search');
            ...
            if(array_key_exists('doSearch',$parameter)){
                    if( $membership_feature_search =='Search by Membership feature tittle')
                    {
                            $membership_feature_search ='';
                    }
                    if(!empty($membership_feature_search)){
                        $likeArr['feature_name'] = urldecode(trim($membership_feature_search));   //<---##                     
                        $searchLink .= "/membership_feature_search/$membership_feature_search";
                        $like = array('feature_name'=>$membership_feature_search);                  //<---##
            ...
            $condition ="status != 2";
            $order_array = $this->ranking->Prepare_OriginalArray('membership_feature','feature_id','feature_order',$condition);
            if(array_key_exists('type',$parameter)){
                    $alter_array = $this->ranking->AlterOriginalArray($order_array,$parameter['order'],$parameter['type']);
                    $this->ranking->saveRanking($alter_array,'membership_feature','feature_id','feature_order');      
            }
            ...
            if($action == 'delete'){
				//echo "hiii";exit;
                $this->ranking->deleteRanking($order_array,'membership_feature','feature_id','feature_order',$record_id);    //<---##
                $Is_Process = $this->input->post('Is_Process');
                $msgreport = $this->membership_feature_model->deletePlan($record_id);
                //$msg = displayMessage($msgreport);				
                $data['status_massage'] = 'Successfully Deleted';
            }

            if($action == 'activate'){
                     $this->membership_feature_model->ChangeStatus($record_id);
            }
            ...
            $SortingSequenceArr=array(1 => "feature_name");          
            $ReturnSortingArr=$this->sorting->Sorting("feature_order",$SortingSequenceArr,'GO',$OrderByID,$ordertype);  
            ...
            $config_pagination['total_rows'] = $this->membership_feature_model->get_TotalRecords($where,$like);
            ...
            //main query
            $CatalogArr = $this->membership_feature_model->get_AllCatalogArr($parameter['page'],$config_pagination['per_page'],$ReturnSortingArr['OrderBy'],$ordertype,$searchArr,$likeArr);

            // Ranking Starts here
            $order_array = $this->ranking->Prepare_OriginalArray('membership_feature','feature_id','feature_order',$condition);   
            for($i=0;$i<count($CatalogArr);$i++){
                    if($searched == '' && !array_key_exists('OrderByID',$parameter))
                            $CatalogArr[$i]->img_rank = $this->ranking->displayRankingImage($order_array,$CatalogArr[$i]->feature_order,base_url().'superadmin/membership_feature/index/page/'.$parameter['page']);         //<---##
                    else
                            $CatalogArr[$i]->img_rank = "--";
            }
            // Ranking Ends here

            //$data['selected_val'] = $this->membership_feature_model->get_curr();
            ...
            $data['membership_feature_search'] = $membership_feature_search;
            $data['searchLink'] = $searchLink;
			//$data['get_All_Currency'] = $this->membership_feature_model->get_All_Currency();
            ...
            $this->load->view('superadmin/membership_feature/membership_feature',$data);
                if($Is_Process!='Y') {
	...
	public function Get_title_Ajax(){
		$this->membership_feature_model->get_title();
	}
	public function SaveAjax(){
		$return=$this->membership_feature_model->Save();
		echo $return;
	}
	public function EditAjax(){
		$ret = $this->membership_feature_model->Edit();
		echo $ret;
	}	
}