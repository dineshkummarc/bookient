...
class Alert_Report_model extends CI_Model
{
	public $_resultArr;
	...
	public function getSms(){
	    ...
	    $this->db->select('*');
        $this->db->from('app_cron_manager');
		if($post['date_from'] !='' && $post['date_to'] !=''){
		   	$date_from 	= date("Y-m-d",strtotime($this->input->post('date_from')));
       		$date_to 	= date("Y-m-d",strtotime($this->input->post('date_to')));
       		
       		
			$this->db->where('cron_executed_datetime >=', $date_from);
			$this->db->where('cron_executed_datetime <=', $date_to);
		}
		
		if($post['local_admin_id'] !=''){
			$this->db->where('cron_local_admin_id', $post['local_admin_id']);
		}
		
		if($post['sent_to'] !=''){
			$this->db->where('cron_customer_email', $post['sent_to']);
		}
		if($post['phone_no'] !=''){
			$this->db->where('cron_customer_mobile', $post['phone_no']);
		}
		$this->db->where('cron_messages_status', 3);
		$query = $this->db->get();
