<?php
class Staff_Manage_model extends CI_Model 
{
	...
	public function add_block_date_data($date_from, $date_to, $blk_dt_employee_id)
	{
            ...
            else
            {
                ...
                if($NumRows == 0)
                {// IF RECORD IS INSERTED FOR FIRST TIME
                    $DateArrCounter = 0;
                    $DateArr = array();
                ...
                {// IF RECORD IS INSERTED FOR NEXT TIME
                    $DateArr = array();
	...
	public function fetch_blocked_timings($id)
	{
            ...
            $arr = $this->global_mod->staffBlockingStorePro();
            $NumRows = 0;
            foreach($arr as $val){
            ...
            if($NumRows > 0){    
                ...
                foreach($arr as $data){
                    if($data['unavailable_time_id'] != 0){
                        ...
                        $RetString .= "<tr>
                                        <td>".$data['block_date']."</td>
                                        <td>".$data['time_form']."</td>
                                        <td>".$data['time_to']."</td>
                                        <td><a href=\"javascript:void(0);\" onclick=\"DeleteTimeBlockData(".$data['unavailable_time_id'].",".$data['employee_id'].");\">Delete</a></td>
                                       </tr>";
                    }
		}
                $RetString .= "</table></div>";
