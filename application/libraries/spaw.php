<?php
	...
	class Spaw extends SpawEditor {
		...
		function Spaw($config) {
			...
			parent::SpawEditor($this->instance, $this->content, '', '', '', '', '', '', $this->caption);
			// load plugin configs
							...
							if(is_dir($spaw_pgdir.$spaw_pg.'/config')) {
								if($spaw_pgdh = opendir($spaw_pgdir.$spaw_pg.'/config')) {
									while (($spaw_fn = readdir($spaw_pgdh)) !== false) {
										if ($spaw_fn != '.' && $spaw_fn != '..' && !is_dir($spaw_pgdir.$spaw_pg.'/config/'.$spaw_fn)) include($spaw_pgdir.$spaw_pg.'/config/'.$spaw_fn);
									}
									closedir($spaw_pgdh);
								}
							}
		...
		/**
		* Returns editor's HTML code to the client
		*/
		function show() {
			return $this->getHtml();
		}
		...
		/**
		* Add additional tabs to the Editor instance
		*/
		function addTab($name, $caption, $value = '') {
			$this->SpawEditor($name, $value, $lang='', $toolbarset='', $theme='', $width='', $height='', $stylesheet='', $caption);
		}
	} 