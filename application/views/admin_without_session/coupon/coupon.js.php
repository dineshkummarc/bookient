...
$(document).ready(function(){
    ...
    $('#submit_coupon').click(function(e){
        ...
        if(discountAmtSett == 1 && discountAmt > 100){
            $('#cd_err').html('Discount amount should be less 100%');
            errorcount++;
            return false;
        }
        ...
        if(time1 != "" && time2 != ""){
            if(hourDiff < 0){
                $('#err_tim').html('End time should be grater than Start time.');
                errorcount++;
                return false;
            }else if(hourDiff == 0){
                if(minDiff <= 0){
...
function change_status_to_on(id,identifier)
{
    ...
    //alert(parentclass);  return false;
    ...
    $.ajax({
        ...
        success: function(msg) {
            if(msg == 1){
                ...
                //$("#coupDispDivId"+id).attr("class","blue-bg-coupon ActiveD");
