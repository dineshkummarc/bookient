<script type="text/javascript" src="<?php echo base_url(); ?>asset/staff_js/staff_calender.js"></script>
<div class="top-controler floatLeft">
<div class="colwidth-top"><div id="radioset">
			...
			<input type="hidden" id="Show_all_booking_date" value="<?php echo $my_array =$this->staff_calender_model->service_date();?>"/>
...
<?php
	...
	$booking_array = $this->staff_calender_model->getSelectedBookingAjax_pr($current_date);
	$WorkableTimArr = $this->staff_calender_model->getWorkingTime();
	for($s=0;$s<count($staff);$s++){
?>
			  <div class="onecol">
				<div class="relative"><div class="employee_name"><?php echo $staff[$s]['employee_name']; ?>
<?php
		$count_staff_booking	=	$this->staff_calender_model->count_staff_booking($staff[$s]['employee_id'],$current_date);
...
?>
			<div class="onerowcolor" style="background:<?php echo  ($i%2)?'':'#e9e9e9';?>" id="scroll_<?php echo $i; ?>">
			  <div class="bodyonecol"><span style="color:#446688; "><?php echo $this->staff_calender_model->set_time($i);?></span></div>
			  <?php 
		   for($s=0;$s<count($staff);$s++){
?>
		...
		if(count($booking_array)>0){
			...
			$chkWorkableTime = $this->staff_calender_model->checkWorkingTime($staff[$s]['employee_id'],$current_date,$currentTime, $WorkableTimArr);
