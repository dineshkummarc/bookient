...
$(document).ready(function(){ 
	...
	$('#btn-submit-login').click(function(e){
        ...
        if ($('#remember').is(':checked')) {
			...
			$.ajax({
			   type: 'POST',
			   url: '<?php echo site_url("staff/staff_login/StaffLogInAjax"); ?>',
			   data: {'user_email': user_email, 'password': password },
			   success: function(data){
				  if($.trim(data) == 0){
					location.href="<?php echo base_url(); ?>staff/staff_calender";
				  }else if($.trim(data) == 2){
					  window.location.href='<?php echo base_url(); ?>staff/login/emailNotVerified';
				  }else{
