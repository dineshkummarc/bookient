...
$(document).ready(function(){
	...
	if (len >= 410) {
		$('#Limit0').text("0 "+"<?php echo $this->lang->line('char_remning');?>");
	}else {
		$('#Limit0').text(410 - len+" <?php echo $this->lang->line('char_remning');?>");
	}
	
	value="<?php echo ($SocialMsg !='')?$SocialMsg[0]['twitter_msg_body']:''; ?>";
	var len = value.length;
	if (len >= 120) {
		$('#Limit1').text("0 "+"<?php echo $this->lang->line('char_remning');?>");
	}else {
		$('#Limit1').text(120 - len+" <?php echo $this->lang->line('char_remning');?>");
	}
